...
def test_the_one_browser_fixture_is_session_scoped_beside_the_evidence_hook():
    ...
    # The conftest's whole surface, by name: the one fixture and the
    # evidence machinery the release gate arms — nothing else may grow here.
    assert re.findall(r"^def (\w+)", source, re.MULTILINE) == [
        "chromium", "_evidence_stem", "_live_pages",
