...
The harness suite already holds that a planted secret misses everything the
ADAPTER can hand onward: its receipt, its verification, its own attributes, and
every file left under the project root. That is a wide sweep and it stops where
the adapter stops. Between the adapter and a reader sit surfaces this build owns
but the adapter never touches -- the journal appended to disk, the bytes a real
server writes to a real socket, the frames pushed to every attached listener --
and a secret that reached one of those would be retained and served while every
adapter-local assertion stayed green.
...
CREDENTIAL is one an operator legitimately allows through ``env_allow``: a fresh
``DSH_HOME`` per attempt means the harness starts with no session of its own, so
a real deployment passes a key by name and this build forwards it -- and
``DSH_API_KEY`` is the very name the harness suite plants on the DENIED side, so
between the two files the allowlist is held in both directions with one spelling.
The PRINTED secret is what the child writes to stdout and stderr. The third is
the same string planted where a VERSION belongs, which is the refusal road: the
token is compared and never carried, not even into the sentence that refuses.
...
is the parity Grok has and dsh did not. What each test buys is what it would say
if a future change made one of these surfaces start carrying what the child was
handed.
"""
...
from tests.test_command_dsh_harness import NOW, a_harness, a_request, run_once
from tests.test_store import good_lane, write_project

#: The credential-shaped name this provider's suites already use. The harness
#: suite plants it in the ambient environment WITHOUT allowing it, and proves it
#: never arrives; here it is allowed, so it does arrive, and the question
#: becomes what this build does with a value it was asked to forward.
CREDENTIAL_ENV = "DSH_API_KEY"
...
CREDENTIAL = "sk-dsh-SYNTHETIC-NEVER-REAL-00000000000000"
...
PRINTED = "PRINTED-BY-THE-CHILD-dsh-0f1e2d3c4b5a6978"
RUN_ID = "run-dsh-leak"
INSTANCE = "dsh-dev"
CONFIG = {
    "cycle": {"id": "dsh-orbit", "phases": ["dispatch"]},
    "instances": [{"id": INSTANCE, "adapter": "deepseek-harness"}],
}
ARGUMENTS = {
...
def _fresh(tmp_path: Path, name: str) -> Path:
    """A directory this suite owns, made before the harness reaches for it.

    The dsh harness builder makes its root with `mkdir(exist_ok=True)` and no
    `parents`, so a caller that hands it a nested path has to have made the
    parent itself. Two harnesses stand up in one test below, which is why each
    one gets a name rather than sharing `tmp_path`.
    """
    made = tmp_path / name
...
def _served(tmp_path: Path, **knobs: str):
    """One dsh dispatch, and a REAL loopback server over the same project.
    ...
    adapter, root, log = a_harness(
        _fresh(tmp_path, "harness"),
        **{_fakedsh.WRITE_FILE: WRITTEN, CREDENTIAL_ENV: CREDENTIAL, **knobs})
...
def test_the_dispatch_this_suite_searches_really_ran_and_really_was_served(
    ...
    assert task["argv"][:2] == ["--profile", "headless"]
    assert len(task["argv"]) == 3, f"UNEXPECTED_ARGV={task['argv']}"
    ...
    homes = [row["dsh_home"] for row in rows]
...
def test_a_secret_the_child_prints_reaches_no_raw_surface(tmp_path):
    ...
    surfaces, _attempt, _root = _surfaces(
        tmp_path, FAKEDSH_STDOUT=PRINTED, FAKEDSH_STDERR=PRINTED)
...
def test_a_secret_planted_where_a_version_belongs_reaches_no_raw_surface(
    ...
    """
    surfaces, _attempt, _root = _surfaces(tmp_path, FAKEDSH_VERSION=PRINTED)

    rows = _rows(surfaces["spawn-log"])
    assert [row for row in rows if row["argv"][:1] != ["--version"]] == [], (
        "a task ran on an unreviewed build")
    leaked = sorted(
        name for name, raw in surfaces.items() if PRINTED.encode() in raw)
...
def test_the_surfaces_still_carry_the_identifiers_a_reader_needs(tmp_path):
    ...
    for owed in ("--profile", "headless", DSH_HOME_ENV,
                 DSH_TELEMETRY_DISABLED_ENV):
...
def test_the_journal_carries_the_runtimes_own_words_and_not_the_adapters(
    ...
    adapter, _root, _log = a_harness(
        _fresh(tmp_path, "direct"), FAKEDSH_VERSION=PRINTED)
    direct = run_once(adapter, a_request())
    with _served(tmp_path / "run", FAKEDSH_VERSION=PRINTED) as (
    ...
    assert REVIEWED_DSH_VERSION in direct.detail, (
    ...
    assert REVIEWED_DSH_VERSION not in journalled, (
...
def test_the_spawn_log_stores_names_and_booleans_and_never_a_value(tmp_path):
    ...
    for row in rows:
        assert set(row) == {
            "argv", "cwd", "dsh_home", "telemetry_disabled", "env_names",
            "probe"}, sorted(row)
