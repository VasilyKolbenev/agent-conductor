...
def test_all_deep_vocabularies_and_protocol_flags_are_exactly_pinned():
    assert set(DeepProtocol) == {
        ...
        DeepProtocol.DSH_HEADLESS_V1}
    assert dict(DEEP_PROTOCOL_FLAGS) == {
        "fake-claude-jsonl-v1": "--fake-claude-jsonl-v1",
        "fake-codex-jsonl-v1": "--fake-codex-jsonl-v1",
    }
    # The dsh token is reviewed vocabulary, not a deep fake protocol: it binds no
    # deep argv flag, so the fake-protocol command builder can never render it.
    assert DeepProtocol.DSH_HEADLESS_V1.value not in DEEP_PROTOCOL_FLAGS
    assert set(DEEP_PROTOCOL_FLAGS) == {
        protocol.value for protocol in DeepProtocol
        if protocol is not DeepProtocol.DSH_HEADLESS_V1}
