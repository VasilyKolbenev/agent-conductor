...
def test_the_panel_registers_no_harness_the_registry_has_not():
    # The other half of the copy guard: adding a product to the panel alone
    # would ship a registry change through the back door. Held as a set relation
    # rather than by naming today's entries, so the next registry change is
    # covered without editing this test.
    assert set(panel_harness_names()) == {entry.id for entry in harnesses.known()}


@pytest.mark.parametrize("name", ["Some Local Agent", "future-harness",
