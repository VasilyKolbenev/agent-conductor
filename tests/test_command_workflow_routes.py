...
# --- the transport contract, for every new route --------------------------
...
def test_publishing_can_never_rewrite_a_revision_that_already_stands(tmp_path):
    ...
    post(subject, f"/command/workflows/{WORKFLOW}/revisions", {"revision": 2})
    post(subject, f"/command/workflows/{WORKFLOW}/draft",
         INCOMPLETE["a dangling edge"])
    post(subject, f"/command/workflows/{WORKFLOW}/revisions", {"revision": 3})
