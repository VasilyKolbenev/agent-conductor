...
because that helper is what the claim is about.
...
The harness registry's half of the wizard — the menu, the custom row, and the
rule that only an id ever reaches `map.toml` — lives in
test_init_harness_menu.py, which imports `_init_args`, `_scripted`,
`_unwrapped`, `_map_text` and `_harnesses` from here rather than keeping a
...
from conductor.init import (_FIRST_ACTION, _FIRST_ACTION_NO_PLACEHOLDERS,
                            _console_ask, _first_action, _interactive)
