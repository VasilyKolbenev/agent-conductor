...
The Job Object is created and the child assigned to it immediately after
``CreateProcess`` returns and before the child has run any Python, so a
grandchild the child later spawns is already inside the job. The POSIX branch
relies on ``start_new_session`` making the child a session and process-group
leader, so ``killpg`` reaches the whole session.
"""
...
if os.name == "nt":  # pragma: win32 cover
    ...
    class _HandleGroup:
        """Fallback when a Job is unavailable: terminate the exact process handle."""
    ...
    def make_group(proc: "subprocess.Popen[bytes]") -> ProcessGroup:
        ...
        if job and _k32.AssignProcessToJobObject(job, int(proc._handle)):
            return _JobGroup(job, proc)
        if job:
            _k32.CloseHandle(job)
        return _HandleGroup(proc)
