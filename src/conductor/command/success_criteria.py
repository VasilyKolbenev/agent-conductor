...
def criteria_rows(
        *, kind: object, capability: object, verifier: object, doer: object,
        ...
        arguments: Mapping[str, Any] | None) -> tuple[dict[str, str], ...]:
    ...
    rows = [
        {"text": "The result must be verified. A step that finished is not a "
                 "step that succeeded, and an unverified result never counts "
                 "as one.",
         "source": VERIFIED_SOURCE},
        _verified_by(verifier, doer),
    ]
    if capability == REVIEW_CAPABILITY:
        rows.append(_publishes(held))
    if required_evidence in REQUIRED_EVIDENCE:
        rows.append(
            {"text": f"The verification must name a {required_evidence}. This "
                     "plan asks for more proof than the runtime demands of "
                     "every step, and a verification without it is refused.",
             "source": DEMAND_SOURCE})
    return tuple(rows)


def _verified_by(verifier: object, doer: object) -> dict[str, str]:
    ...
    if isinstance(verifier, str) and verifier:
        return {"text": f"Verified by {verifier}, which this plan names as "
    ...
    if isinstance(doer, str) and doer:
        return {"text": f"Verified by {doer}, the instance that does the work "
                        "— this plan names no separate verifier.",
                "source": PLAN_SOURCE}
    return {"text": "Verified by the instance that does the work. This step "
                    "binds none yet, so there is nobody to name.",
            "source": PLAN_SOURCE}


def _publishes(arguments: Mapping[str, Any]) -> dict[str, str]:
    ...
    if named is None:
        return {"text": "A review must publish its result as a durable "
                        "artifact. This step names no result reference yet, so "
                        "there is nothing for one to answer.",
                "source": CHAIN_SOURCE}
    return {"text": f"The artifact {named} must stand, published by this "
                    "step's own action and answering the request that asked "
                    "for it — the same reference, from the inputs it named.",
            "source": CHAIN_SOURCE}


def for_plan_node(node: Any) -> tuple[dict[str, str], ...]:
    ...
    return criteria_rows(
        ...
        required_evidence=node.required_evidence, arguments=node.arguments)


def for_template_node(node: Any) -> tuple[dict[str, str], ...]:
...
def for_plan(nodes: Any) -> dict[str, list[dict[str, str]]]:
    """Every step of one plan, keyed by node id, for a read to carry."""
    return {node.node_id: [dict(row) for row in for_plan_node(node)]
            for node in nodes}


def for_document(nodes: Any) -> dict[str, list[dict[str, str]]]:
    """Every step of one drawing, keyed by node id, for a read to carry."""
    return {node.node_id: [dict(row) for row in for_template_node(node)]
            for node in nodes}
