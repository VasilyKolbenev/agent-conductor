...
export function badge(registry, harnessId) {
  ...
  const swatch = element("span", {className: "hb__m", text: monogram});
  if (row) swatch.style.setProperty("--hb-dark", row.dark);
  if (row) swatch.style.setProperty("--hb-light", row.light);
  return element("span", {className: "hb"}, [
    swatch, element("span", {className: "hb__n", text: name}),
  ]);
}

function chip(channel, text) {
  return element("span", {className: `g-chip g-chip--${channel}`, text});
}
...
function nodeButton(state, node, onSelect) {
  ...
  if (node.gate) chips.append(
    chip(GATE_CHANNEL[node.gate.state], GATE_GLYPHS[node.gate.state]));
  button.append(chips);
...
function decisionForm(state, node, onDecide) {
  ...
  const action = element("select", {name: "action"},
    Object.keys(DECISION_ACTIONS).map((name) =>
      element("option", {text: name, value: name})));
  const actor = element("input", {autocomplete: "off", maxlength: "128",
    name: "actor", pattern: "[A-Za-z0-9][A-Za-z0-9._\\-]{0,127}", required: "",
    spellcheck: "false", type: "text"});
  const reason = element("input", {autocomplete: "off", maxlength: "200",
    name: "reason", spellcheck: "false", type: "text"});
  form.append(field("Action", action), field("Decided by", actor),
    field("Reason", reason),
    element("button", {text: "Record decision", type: "submit"}));
  form.addEventListener("submit", (event) => {
    event.preventDefault();
    const data = new FormData(form);
    onDecide({gateId: node.gate.gate_id,
...
export function renderDetail(mount, state, onDecide) {
  mount.replaceChildren(element("h2", {text: "Node card"}));
  ...
  if (node.gate) {
    mount.append(chip(GATE_CHANNEL[node.gate.state],
      `${node.gate.gate_id}: ${GATE_GLYPHS[node.gate.state]}`));
    mount.append(decisionForm(state, node, onDecide));
    mount.append(element("p", {"aria-live": "polite",
      className: "g-note g-decide-status", text: state.decisionNotice}));
  }
}
...
export function renderComposer(mount, state, onCompose) {
  mount.replaceChildren(element("h2", {text: "Compose"}));
  if (!state.nodes.length) return;
  const form = element("form", {className: "g-compose"});
  const title = element("input", {autocomplete: "off", maxlength: "80",
    name: "title", required: "", spellcheck: "false", type: "text"});
  const harness = element("select", {name: "harness"}, [
    element("option", {text: "no harness", value: ""}),
    ...state.registry.map((row) =>
      element("option", {text: row.name, value: row.id})),
  ]);
  const anchor = element("select", {name: "anchor"}, state.nodes.map((node) =>
    element("option", {text: node.title, value: node.node_id})));
  const placement = element("select", {name: "placement"}, [
  ...
  form.append(field("Step title", title), field("Harness", harness),
    ...
    element("button", {text: "Add step to fixture", type: "submit"}));
  form.addEventListener("submit", (event) => {
    event.preventDefault();
    const data = new FormData(form);
    onCompose({title: String(data.get("title") || ""),
      harness: String(data.get("harness") || "") || null,
      anchorId: String(data.get("anchor") || ""),
