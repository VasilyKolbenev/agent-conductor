"use strict";
import {
  ERROR_LABELS, RUN_ID, exactArguments, isId, projectControls, projectGates,
  projectProposal, projectRecords, projectScope, refusalCode, safeMode,
} from "./command-projection.js";
import {
  element, gateLabel, gateRow, renderComposer, renderProposalReview,
...
(() => {
  ...
  async function submitProposal(event) {
    ...
    const proposal = projectProposal(payload, submitted, state.runId);
    if (!proposal) {
      state.proposalPhase = "outcome-unknown";
      state.proposalNotice = "Outcome unknown. Reload the authoritative run.";
      render();
